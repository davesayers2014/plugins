...
defaultFileLocation = "https://raw.githubusercontent.com/davesayers2014/AutoBouquetsMaker/master/AutoBouquetsMaker/custom/cable_uk_virgin_CustomMix.xml"
ABMpath = "/usr/lib/enigma2/python/Plugins/SystemPlugins/AutoBouquetsMaker/custom/"
ABMfile = "cable_uk_virgin_CustomMix.xml"

config.plugins.SatFTAImporter = ConfigSubsection()
config.plugins.SatFTAImporter.fileLocation = ConfigText(default = defaultFileLocation, fixed_size = False)
config.plugins.SatFTAImporter.enableImporter = ConfigYesNo(default = True)
config.plugins.SatFTAImporter.leadTime = ConfigSelection(default = "5", choices = [("1", _("1 minute")), ("2", _("2 minutes")), ("3", _("3 minutes")), ("5", _("5 minutes")), ("10", _("10 minutes")), ("20", _("20 minutes")), ("30", _("30 minutes"))])

class SatFTACustomMixImporterScreen(Setup):
	skin = """
	...
	def __init__(self, session, setup, plugin=None, menu_path=None, PluginLanguageDomain=None):
		...
		self.skinName = ["SatFTACustomMixImporterScreen", "Setup4buttons"]
	...
	def saveConfig(self):
		config.plugins.SatFTAImporter.save()
		configfile.save()
	...
	def startImporter(self):
		self.session.openWithCallback(self.startImporterCallback, SatFTACustomMixImporter)

	def startImporterCallback(self, answer = None):
		if answer:
			self.close()

class SatFTACustomMixImporter(Screen):
	skin = """
	...
	def __init__(self, session):
		...
		Screen.setTitle(self, _("SatFTA CustomMix"))
	...
	def firstExec(self):
		...
		if CustomMix:
			try:
				...
				print "[SatFTACustomMixImporter]Saving file failed."

	def fetchURL(self):
		try:
			req = urllib2.Request(config.plugins.SatFTAImporter.fileLocation.value)
			response = urllib2.urlopen(req)
			print '[SatFTACustomMixImporter][fetchURL] Response: %d' % response.getcode()
			if int(response.getcode()) == 200:
				return response.read()
		except urllib2.HTTPError, err:
			print '[SatFTACustomMixImporter][fetchURL] ERROR:',err
		except urllib2.URLError, err:
			print '[SatFTACustomMixImporter][fetchURL] ERROR:',err.reason[0]
		except urllib2, err:
			print '[SatFTACustomMixImporter][fetchURL] ERROR:',err
		except:
			import sys
			print '[SatFTACustomMixImporter][fetchURL] undefined error', sys.exc_info()[0]
...
class schedule:
	instance = None
	def __init__(self, session):
		print "[SatFTACustomMixSchedule][__init__] Starting..."
		self.session = session
		self.justBootedOrConfigChanged = True
		self.enableImporter = config.plugins.SatFTAImporter.enableImporter.value
		self.leadTime = config.plugins.SatFTAImporter.leadTime.value
		self.fileLocation = config.plugins.SatFTAImporter.fileLocation.value
		try:
			...
			print "[SatFTACustomMixSchedule][__init__] ABM config available"
		except:
			self.enableSchedule = False
			self.clock = [0,0]
			self.repeattype = "daily"
			print "[SatFTACustomMixSchedule][__init__] ABM config was not available"
		...
		assert simpleSchedule.instance is None, "[SatFTACustomMixImporter] class simpleSchedule is a singleton class and just one instance of this class is allowed!"
	...
	def configChecker(self):
		if self.enableImporter != config.plugins.SatFTAImporter.enableImporter.value or \
			self.leadTime != config.plugins.SatFTAImporter.leadTime.value or \
			self.fileLocation != config.plugins.SatFTAImporter.fileLocation.value or \
			...
			self.repeattype != config.autobouquetsmaker.repeattype.value \
		:
			print "[SatFTACustomMixImporter][configChecker] config has changed"
			self.enableImporter = config.plugins.SatFTAImporter.enableImporter.value
			self.leadTime = config.plugins.SatFTAImporter.leadTime.value
			self.fileLocation = config.plugins.SatFTAImporter.fileLocation.value
			self.enableSchedule = config.autobouquetsmaker.schedule.value
	...
	def doSchedule(self):
		...
		if self.enableSchedule and self.enableImporter:
			...
			print "[SatFTACustomMixImporter][doSchedule] Scheduler disabled."
	...
	def startNextCycle(self):
		...
		if next > 0:
			...
			next = -1
		print "[SatFTACustomMixImporter][startNextCycle] Time set to", strftime("%c", localtime(next)), strftime("(now=%c)", localtime(now))
...
def pluginAutoStart(reason, session=None, **kwargs):
	...
	now = int(time())
	if reason == 0:
		print "[SatFTACustomMixImporter][pluginAutoStart] AutoStart Enabled"
		if session is not None:
			_session = session
			if scheduleTimer is None:
				scheduleTimer = schedule(session)
	else:
		print "[SatFTACustomMixImporter][schedule] Stop"
		scheduleTimer.stop()
...
def taskToSchedule(session, **kwargs):
	session.open(SatFTACustomMixImporter)

def pluginManualStart(menuid, **kwargs):
	if menuid == "scan":
		return [(_("SatFTA CustomMix"), SatFTACustomMixImporterMain, "SatFTACustomMixImporterScreen", 11)]
	return []

def SatFTACustomMixImporterMain(session, **kwargs):
	menu_path = "%s / %s / %s" % (_('Main menu'), _('Setup'), _('Service searching'))
	session.open(SatFTACustomMixImporterScreen, 'SatFTAcustommiximporter', 'SystemPlugins/SatFTACustomMixImporter', menu_path, PluginLanguageDomain)

def Plugins(**kwargs):
	pList = []
	if ABMisLoaded():
		pList.append( PluginDescriptor(where = [PluginDescriptor.WHERE_SESSIONSTART], fnc=pluginAutoStart))
		pList.append( PluginDescriptor(name=_("SatFTA CustomMix Importer"), description="Imports SatFTA CustomMix for ABM", where = PluginDescriptor.WHERE_MENU, fnc=pluginManualStart, needsRestart=True) )
